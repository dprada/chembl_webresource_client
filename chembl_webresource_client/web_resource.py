...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def _get_asynchr(self, kname, keys, frmt='json', prop=None, retry=0):
        try:
          rs = (self.get_one(**{'frmt': frmt, kname:key, 'asynchr': True, 'prop': prop})
	      for key in keys)
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def get_asynchr(self, kname, keys, frmt='json', prop=None):
        ret = self._get_asynchr(kname, keys, frmt, prop)
    ...
    def _get(self, kname, keys, frmt='json', prop=None):
        if isinstance(keys, list):
            if len(keys) > Settings.Instance().ASYNC_TRESHOLD and grequests:
                return self.get_asynchr(kname, keys, frmt, prop)
            else:
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def _get_one(self, url, asynchr, frmt, method='get', data=None):
        with self._get_session() as session:
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def get_one(self, chembl_id, frmt='json', asynchr=False, prop=None):
        if chembl_id:
            if not prop:
                ...
                url = '{0}/{1}/{2}/{3}.{4}'.format(Settings.Instance().webservice_root_url, self.name,
                                          chembl_id, prop, frmt)
            return self._get_one(url, asynchr, frmt)
